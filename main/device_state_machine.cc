...
bool DeviceStateMachine::IsValidTransition(DeviceState from, DeviceState to) const {
    ...
    switch (from) {
        ...
        case kDeviceStateIdle:
            // Can go to connecting, listening (manual mode), speaking, music playing, activating, upgrading, or wifi configuring
            return to == kDeviceStateConnecting ||
                   to == kDeviceStateListening ||
                   to == kDeviceStateSpeaking ||
                   to == kDeviceStateMusicPlaying ||
                   to == kDeviceStateActivating ||
                   to == kDeviceStateUpgrading ||
                   to == kDeviceStateWifiConfiguring;

        case kDeviceStateConnecting:
            // Can go to idle (failed), listening (success), or music playing
            return to == kDeviceStateIdle ||
                   to == kDeviceStateListening ||
                   to == kDeviceStateMusicPlaying;

        case kDeviceStateListening:
            // Can go to speaking, music playing, or idle
            return to == kDeviceStateSpeaking ||
                   to == kDeviceStateMusicPlaying ||
                   to == kDeviceStateIdle;

        case kDeviceStateSpeaking:
            // Can go to listening, music playing, or idle
            return to == kDeviceStateListening ||
                   to == kDeviceStateMusicPlaying ||
                   to == kDeviceStateIdle;

        case kDeviceStateMusicPlaying:
            // 禁止进入 Listening：音乐播放中麦克风上行到服务器，VAD 可能误将音乐当语音触发状态切换
            // 用户按 BOOT 键走 Idle（先 StopMusic 再切 Idle），不需要直接切 Listening
            return to == kDeviceStateIdle;
