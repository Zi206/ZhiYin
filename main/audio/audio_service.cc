...
void AudioService::Start() {
    ...
    xTaskCreatePinnedToCore([](void* arg) {
        ...
        vTaskDelete(NULL);
    }, "opus_codec", 2048 * 12, this, 2, &opus_codec_task_handle_, 0);
}
...
void AudioService::OpusCodecTask() {
    while (true) {
        ...
        /* Decode audio from decode queue — 批量处理，减少调度开销 */
        int decoded_count = 0;
        while (!audio_decode_queue_.empty() && audio_playback_queue_.size() < MAX_PLAYBACK_TASKS_IN_QUEUE) {
            ...
            if (opus_decoder_ != nullptr) {
                ...
                if (ret == ESP_AUDIO_ERR_OK) {
                    ...
                    lock.lock();
                    break;  // Decode failure, stop batch
                }
            } else {
                ESP_LOGE(TAG, "Audio decoder is not configured");
                lock.lock();
                break;  // No decoder, stop batch
            }
            debug_statistics_.decode_count++;
        }
