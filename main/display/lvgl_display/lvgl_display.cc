...
void LvglDisplay::SetStatus(const char* status) {
    ...
    if (notification_label_ != nullptr) {
        lv_anim_delete(notification_label_, nullptr);
        lv_obj_set_style_opa(notification_label_, LV_OPA_COVER, 0);
        lv_obj_set_style_translate_y(notification_label_, 0, 0);
    }
...
void LvglDisplay::ShowNotification(const char* notification, int duration_ms) {
    ...
    // One finite animation owns fade-in, hold, and fade-out. This keeps
    // notification motion deterministic without another continuously running
    // timer or animation object.
    lv_anim_t notification_anim;
    lv_anim_init(&notification_anim);
    lv_anim_set_var(&notification_anim, notification_label_);
    lv_anim_set_values(&notification_anim, 0, 1000);
    lv_anim_set_time(&notification_anim, duration_ms < 400 ? 400 : duration_ms);
    lv_anim_set_path_cb(&notification_anim, lv_anim_path_linear);
    lv_anim_set_user_data(&notification_anim, this);
    lv_anim_set_custom_exec_cb(&notification_anim, [](lv_anim_t* anim, int32_t value) {
        auto* label = static_cast<lv_obj_t*>(anim->var);
        int32_t opacity = 255;
        int32_t translate_y = 0;
        if (value < 100) {
            opacity = value * 255 / 100;
            translate_y = -6 + value * 6 / 100;
        } else if (value > 900) {
            opacity = (1000 - value) * 255 / 100;
        }
        lv_obj_set_style_opa(label, static_cast<lv_opa_t>(opacity), 0);
        lv_obj_set_style_translate_y(label, translate_y, 0);
    });
    lv_anim_set_completed_cb(&notification_anim, [](lv_anim_t* anim) {
        auto* display = static_cast<LvglDisplay*>(anim->user_data);
        if (display == nullptr || display->notification_label_ == nullptr) {
            return;
        }
        lv_obj_add_flag(display->notification_label_, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_style_opa(display->notification_label_, LV_OPA_COVER, 0);
        lv_obj_set_style_translate_y(display->notification_label_, 0, 0);
        if (display->status_label_ != nullptr) {
            lv_obj_remove_flag(display->status_label_, LV_OBJ_FLAG_HIDDEN);
        }
    });
    lv_anim_start(&notification_anim);
}
        ...
        if (low_battery_popup_ != nullptr && !update_all) {
            if (strcmp(icon, FONT_AWESOME_BATTERY_EMPTY) == 0 && discharging) {
                if (lv_obj_has_flag(low_battery_popup_, LV_OBJ_FLAG_HIDDEN)) { // Show if low battery popup is hidden
                    ...
                    lv_anim_delete(low_battery_popup_, nullptr);
                    lv_anim_t pulse;
                    lv_anim_init(&pulse);
                    lv_anim_set_var(&pulse, low_battery_popup_);
                ...
                if (!lv_obj_has_flag(low_battery_popup_, LV_OBJ_FLAG_HIDDEN)) { // Hide if low battery popup is shown
                    lv_anim_delete(low_battery_popup_, nullptr);
                    lv_obj_set_style_opa(low_battery_popup_, LV_OPA_COVER, 0);
