...
#else
    if(0 == (INT_MAX - sizeof(gd_GIF)) / width / height / 4){
        ESP_LOGW(TAG, "Image dimensions are too large");
        goto fail;
    }
...
#else
    // RGB565 canvas occupies two bytes per pixel. The old four-byte loop
    // overwrote the adjacent indexed-frame workspace before frame decoding.
    memset(gif->canvas, 0, 2 * width * height);
#endif
    ...
    for(j = 0; j < gif->fh; j++) {
        for(k = 0; k < gif->fw; k++) {
            ...
            if(!gif->gce.transparency || index != gif->gce.tindex) {
                ...
                uint8_t g = *(color + 1);
                uint8_t b = *(color + 2);
                // 暖色滤镜: R+8%, B-8% → 直接输出RGB565
                int tr = (r * 276) >> 8; r = tr > 255 ? 255 : (uint8_t)tr;
                int tb = (b * 235) >> 8; b = (uint8_t)tb;
                out[i + k] = (uint16_t)(((r & 0xF8) << 8) | ((g & 0xFC) << 3) | (b >> 3));
            }
        }
        i += gif->width;
    }
...
#else
            int j, k;
            uint8_t br = *(bgcolor + 0), bg = *(bgcolor + 1), bb = *(bgcolor + 2);
            uint16_t bg_rgb565 = opa ? (uint16_t)(((br & 0xF8) << 8) | ((bg & 0xFC) << 3) | (bb >> 3)) : 0;
            uint16_t* canvas16 = (uint16_t*)gif->canvas;
            for(j = 0; j < gif->fh; j++) {
                for(k = 0; k < gif->fw; k++) {
                    canvas16[i + k] = bg_rgb565;
                }
                i += gif->width;
            }
#endif
